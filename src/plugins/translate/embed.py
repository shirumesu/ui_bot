...
async def process_photo(words: dict, size: tuple, image_path: str, vertical: bool) -> str:
    ...
    for word in data_list:
        if not cacha_word:
            ...
            sentense = word['words']
            continue
        if abs(word['top'] - cacha_word[-1]['top']) <= round(size[0]*0.02) and abs(word['left'] - cacha_word[-1]['left']) <= round(size[1]*0.05):
            sentense += word['words']
            cacha_word.append(word)
            continue
        else:
            ...
            sentense = sentense.replace(
                ',', '') if word['left'] <= word['height'] else sentense
    ...
    for index, i in enumerate(pos_data_for_word):
        ...
        down = (max([x['left'] + x['width'] for x in i]),
...
async def add_text(image: Image.open, words: dict) -> Image:
    ...
    fontstyle = ImageFont.truetype(os.path.join(
        config.res, 'source', 'translate', 'simhei.ttf'), size_of_text, encoding='utf-8')
    ...
    while (width_text*height_text) < text_number:
        words['text_size'] -= 5
        width_text = abs(words['top'][0] - words['down']
                         [0]) / words['text_size']
        height_text = abs(words['top'][1] - words['down']
                          [1]) / words['text_size']

    down_tigg = words['text_size']+5
    for x, y in enumerate(textwrap.wrap(words['trans'], round(width_text))):
        image_draw.text((words['top'][0], words['top'][1] + x*down_tigg),
                        text=y, fill=(0, 0, 0), font=fontstyle)
    return image
...
async def add_text_for_manga(image: Image.open, words: dict) -> Image.open:
    ...
    total_pixiv_for_text = (
        (y_diff * x_diff) / (words['text_size'] * words['text_size']))
    text_number = len(words['trans'])
    ...
    image_draw = ImageDraw.Draw(image)
    fontstyle = ImageFont.truetype(os.path.join(
        config.res, 'source', 'translate', 'simhei.ttf'), words['text_size'], encoding='utf-8')
    down_tigg = words['text_size'] + 5
    ...
    for right_times, i in enumerate(ttwrap):
        for times, text in enumerate(i):
            image_draw.text((ray[0] - right_tigg*right_times, ray[1] + down_tigg *
                            times), text=text, fill=(0, 0, 0), font=fontstyle)
