...
sv = Service(['reply_msg', '自定义回复'], sv_help,
...
config_path = os.path.join(
    os.getcwd(), 'src', 'plugins', 'reply_msg', 'config.json')
...
async def show_reply(session: CommandSession):
    ...
    reply_msg = '\n'.join(
        [f"{x}: {','.join(y)}" for x, y in self_config[gid].items()])
...
async def del_all_reply(session: CommandSession):
    stat = await Service_Master().check_permission('reply_msg', session.event, GROUP_ADMIN)
    if not stat[0]:
        await session.finish(stat[3])
    msg = session.current_arg.strip()
    del_list = [str(gid)
                for gid in self_config if msg in self_config[str(gid)]]
    if del_list:
        for i in del_list:
...
async def search(Bot, event, plutin_manager):
    ...
    if s:
        await bot.send_group_msg(group_id=gid, message=random.choice(self_config[str(gid)][msg]))
