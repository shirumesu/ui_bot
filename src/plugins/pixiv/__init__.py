...
subcribe_path = os.path.join(os.getcwd(), "src", "plugins", "pixiv", "subcribe.json")
try:
    with open(subcribe_path, "r", encoding="utf-8") as f:
        subcribe = ujson.load(f)
except FileNotFoundError:
    subcribe = {"pixiv": {}, "pixivison": {"group": [], "user": [], "cacha": []}}
...
async def pixiv_rank(session: CommandSession):
    ...
    num = [int(x) for x in re.findall(r"\d+", session.current_arg_text.strip())][0]
...
async def pixivison_page(session: CommandSession):
    ...
    for i in vison_data:
        vison_msg += (
            f"画作标题: {i['image_name']}(id: {i['image_id']})\n"
            f"画师名: {i['user_name']}(id: {i['user_id']})\n"
            f"{i['vison_seq']}\n"
        )
    msg = (
        f"标题: {vison_header['title']}\n"
        f"{vison_header['seq']}\n"
        f"{vison_header['desc']}\n\n"
        f"{''.join(vison_msg)}"
    ).strip()
...
async def pixiv_illuster(session: CommandSession):
    ...
    try:
        ...
        await session.finish("请求api发生错误,无法获取画师精选")
    except Pixiv_not_found_Error:
        await session.finish(
            "获取画师精选图片失败,可能原因:\n"
            "1.画师id错误\n"
            "2.画师没有任何投稿\n"
            "3.Pixiv api返回结果与实际不符(请联系维护组)"
        )
    except:
        await session.finish("发生未知错误,无法获取画师精选,请联系维护组")
...
async def del_illuster(session: CommandSession):
    ...
    if session.event.detail_type == "group":
        gid = str(session.event.group_id)
        if (
            pid not in subcribe["pixiv"]
            or int(gid) not in subcribe["pixiv"][pid]["group"]
        ):
            pid = [x for x, y in subcribe["pixiv"].items() if y["name"] == pid][0]
            if not pid:
        ...
        if (
            pid not in subcribe["pixiv"]
            or int(uid) not in subcribe["pixiv"][uid]["user"]
        ):
            pid = [x for x, y in subcribe["pixiv"].items() if y["name"] == pid][0]
            if not pid:
...
async def check_subcribe_pixiv_list(session: CommandSession):
    if session.event.detail_type == "group":
        ...
        illuster_list = [
            [x, y]
            for x, y in subcribe["pixiv"].items()
            ...
            if isinstance(i, Exception):
                ...
                for gid in illuster["group"]:
                    await bot.send_group_msg(
                        group_id=gid,
                        message=f"检测到{illuster['name']}(id: {i['user_id']})更新\n图片获取失败,id: {new_list[index]}",
                    )
                    logger.error(
