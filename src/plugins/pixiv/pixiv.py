...
async def get_image(pid: str) -> dict:
    ...
    """
    header = {"accept-language": "zh-CN,zh-HK;q=0.9,zh;q=0.8,en;q=0.7"}
    ...
    tags = "，".join(
        [
            f"{x['tag']}({x['translation']['en']})"
            for x in js["tags"]["tags"]
            if "translation" in x
        ]
        + [f"{x['tag']}" for x in js["tags"]["tags"] if "translation" not in x]
    )
    ...
    url = [
        js["urls"]["original"].replace("i.pximg.net", "i.pixiv.cat")
        if config.proxy_pixiv
        else js["urls"]["original"]
    ]
    if count != 1:
...
async def check_illust_list(uid: str) -> dict:
    ...
    if config.pixiv_cookie:
        headers["cookie"] = config.pixiv_cookie
...
@retry(retry_on_exception=(httpx.TimeoutException, RuntimeError))
