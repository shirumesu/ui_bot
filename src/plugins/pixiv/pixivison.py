...
async def get_pixivion_index(return_photo: bool = True) -> list[dict]:
    ...
    """
    # 请求部分
    headers = {"accept-language": "zh-CN,zh-HK;q=0.9,zh;q=0.8,en;q=0.7"}
    ...
    for item in sec:
        ...
        tags.append(
            [
                item.contents[0].next_sibling.next_sibling.contents[0].contents[x].text
                for x in range(
                    len(item.contents[0].next_sibling.next_sibling.contents[0].contents)
                )
            ]
        )
        dates.append(item.contents[0].next_sibling.next_sibling.contents[1].string)
    ...
    thums.append(sec[0].contents[0].contents[0].contents[0].contents[0].attrs["style"])
    titles.append(sec[0].contents[0].contents[1].contents[0].contents[0].string)
    tags.append(
        [
            sec[0].contents[0].contents[1].contents[0].contents[1].contents[x].string
            for x in range(len(sec[0].contents[0].contents[1].contents[0].contents[1]))
        ]
    )
    dates.append(
    ...
    for i in thums:
        ...
        if url:
            url = "https://i.pixiv.cat/img-original" + url[0].replace("_master1200", "")
        else:
            try:
                url = re.findall(r"(https://i.pximg.net/imgaz/upload[\/\d\.\w]*)", i)[0]
    ...
    # 封装字典
    vison_data = []
    for vison_type, url, title, tag, date, image_link in zip(
        types, urls, titles, tags, dates, image_url
    ):
        vison_data.append(
            {
                "type": vison_type,
...
async def get_page_pixivison(url: str) -> Tuple[dict, dict]:
    ...
    try:
        url = re.findall(r"(/img-master/img/.*)", banner_url)[0].replace(
            "_master1200", ""
        )
        if config.proxy_pixiv:
    ...
    vison = soup.find("div", class_="am__body").find_all("div", class_="am__work")
                    ...
                    try:
                        ...
                        try:
                            image_url = re.findall(
                                r"(https://i.pximg.net/c/768x1200_80/img-master/img/[\d\/_\w].(?:jpg|png))",
                                str(item),
                            )
                        except:
                            image_url = "无法获取原图链接"
        image_url = image_url.replace(
            "/c/768x1200_80/img-master", "/img-original"
        ).replace("_master1200", "")
        if config.proxy_pixiv:
    ...
    # 封装data
    vison_data = []
    for title, image_url, image_id, user_name, user_id in zip(
        titles, image_urls, image_ids, user_names, user_ids
    ):
        vison_data.append(
            {
...
async def update_cacha(subcribe: dict, path: str) -> Tuple[dict, dict]:
    ...
    new_list = [
        x
        for x, y in zip(vison_index, md5_list)
        if y not in subcribe["pixivison"]["cacha"]
    ]
