...
sv = Service(['rsshub', '订阅RssHub'], sv_help,
             use_cacha_folder=True, permission_change=GROUP_ADMIN)

bot = get_bot()


subc_path = os.path.join(os.getcwd(), 'src', 'plugins',
                         'rsshub', 'subcribe.json')
...
async def subc_rss(session: CommandSession):
    ...
    if 'twitter' in rss_link or 'pixiv' in rss_link or 'fanbox' in rss_link or 'pcr' in rss_link:
...
async def check_subc(session: CommandSession):
    ...
    if session.event.detail_type == 'group':
        ...
        name_list = [y['name']
                     for x, y in subcribe.items() if gid in y['subcribe_group']]
        status_list = [y['subcribe_group'][gid]
                       for x, y in subcribe.items() if gid in y['subcribe_group']]
        if not name_list:
            await session.finish('本群还没有订阅任何rss！')
        msg = '本群订阅了以下rss:\n'
        for name, status, index in zip(name_list, status_list, [x for x in range(len(name_list))]):
            msg += f"{index + 1}: {name} -> {'[√]详细订阅' if status else '[×]详细订阅'}\n"

    elif session.event.detail_type == 'private':
        uid = str(session.event.user_id)
        name_list = [x['name']
                     for y, x in subcribe.items() if uid in x['subcribe_user']]
        status_list = [x['subcribe_user'][uid]
                       for y, x in subcribe.items() if uid in x['subcribe_user']]
        if not name_list:
            await session.finish('本群还没有订阅任何rss！')
        msg = '你订阅了以下rss:\n'
...
async def del_rss(session: CommandSession):
    ...
    if session.event.detail_type == 'group':
        gid = str(session.event.group_id)
        name_list = [x['name']
                     for y, x in subcribe.items() if gid in x['subcribe_group']]
        status_list = [x['subcribe_group'][gid]
                       for y, x in subcribe.items() if gid in x['subcribe_group']]
        if not name_list:
            await session.finish('本群还没有订阅任何rss！')
        msg = '本群订阅了以下rss:\n'
    ...
    if not re.match(r'\d+', session.current_arg_text.strip()) and int(session.current_arg_text.strip()) <= len(name_list):
...
async def add_config(link: str, full_text: bool = False, gid: int = None, uid: int = None):
    """保存传入的链接
...
async def update_hash(link: str):
    ...
    if js['status'] != 200:
        ...
        md5_list = [md5(str(x).encode('utf-8')).hexdigest()
                    for x in js['entries']]
...
async def check_update(link):
    ...
    if js['status'] != 200:
        subcribe[link]['error_times'] += 1
        raise RuntimeError
    news_list = [x for x in js['entries'] if md5(
        str(x).encode('utf-8')).hexdigest() not in subcribe[link]['hash_cacha']]
    if not news_list:
        return link
    ...
    for item in news_list:
        ...
        b64_image = [x for x in re.findall(
            r'(<img .*?src="data:image/.*?>)', descrip)]
        url_image = [x for x in re.findall(
            r'(<img .*?src="(?:http[s]://[0-9a-zA-Z-./]*.(?:png|jpg)).*?>)', descrip)]
        urls = [x for x in re.findall(
            r'(http[s]://[0-9a-zA-Z-./]*.(?:png|jpg))', descrip)]
        if urls:
...
async def process_b64(b64_encode: str) -> str:
    ...
    b64_encode = re.sub(
        r'data:image/(jpeg|png|jpg)?;base\d+,', '', src)
