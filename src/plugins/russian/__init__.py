...
"""
sv = Service(
    ["russian", "俄罗斯轮盘"], sv_help, permission_change=GROUP_ADMIN, priv_use=False
)
...
# main part
@on_command("rs", aliases=("俄罗斯轮盘", "俄罗斯转盘", "开枪"), only_to_me=False)
async def spin(session: CommandSession):
    ...
    stat = await Service_Master().check_permission("russian", session.event)
    if not stat[0]:
        if stat[3]:
