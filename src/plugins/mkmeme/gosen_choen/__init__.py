...
upper_font_path = path.join(
    config.res, 'source', 'mkmeme', 'gosen_choen', 'FZ_SimHei.ttf')
downer_font_path = path.join(
    config.res, 'source', 'mkmeme', 'gosen_choen', 'FZ_SimSong.ttf')


def _round(f, r=ROUND_HALF_UP): return int(
...
async def get_gradient_2d(start, stop, width, height, is_horizontal=False):
    if is_horizontal:
...
async def getTextWidth(text, font, width=100, height=500, recursive=False):
    step = 100
    ...
    if box[2] < width - step or (recursive and box[2] == width - step):
        ...
        return await getTextWidth(text=text, font=font, width=width+step, height=height, recursive=True)


async def get_gradient_3d(width, height, start_list, stop_list, is_horizontal_list=(False, False, False)):
    result = np.zeros((height, width, len(start_list)), dtype=float)
    for i, (start, stop, is_horizontal) in enumerate(zip(start_list, stop_list, is_horizontal_list)):
        result[:, :, i] = await get_gradient_2d(
            start, stop, width, height, is_horizontal)
    return result


async def createLinearGradient(steps, width, height, size=1):
    height = size*height
    ...
    for i, k in enumerate(steps.keys()):
        ...
        array = await get_gradient_3d(width, h, steps[pk], steps[k])
        result = np.vstack([result, array])
    return result


async def genBaseImage(width=1500, height=500):

    downerSilverArray = await createLinearGradient({
        0: rgb(0, 15, 36),
    ...
    goldArray = await createLinearGradient({
        0: rgb(253, 241, 0),
    ...
    strokeRedArray = await createLinearGradient({
        0: rgb(255, 100, 0),
        0.5: rgb(123, 0, 0),
        0.51: rgb(240, 0, 0),
        1: rgb(5, 0, 0)
    ...
    redArray = await createLinearGradient({
        0: rgb(230, 0, 0),
        0.5: rgb(123, 0, 0),
        0.51: rgb(240, 0, 0),
        1: rgb(5, 0, 0)
    ...
    silver2Array = await createLinearGradient({
        0: rgb(245, 246, 248),
    ...
    navyArray = await createLinearGradient({
        0: rgb(16, 25, 58),
...
async def genImage(word_a="5000兆円", word_b="欲しい!", default_width=1500, height=500,
                   bg="white", subset=250, default_base=None):
    if not word_a and not word_b:
        return path.join(config.res, 'source', 'mkmeme', 'luxun_say', 'sample.png')
    ...
    font_upper = ImageFont.truetype(
        upper_font_path, _round(height/3) + upmargin)
    font_downer = ImageFont.truetype(
        downer_font_path, _round(height/3) + upmargin)

    # Prepare Width
    upper_width = max([default_width,
                      await getTextWidth(word_a, font_upper, width=default_width,
                                         height=_round(height/2))]) + 300
    downer_width = max([default_width,
                       await getTextWidth(word_b, font_upper, width=default_width,
                                          height=_round(height/2))]) + 300
    ...
    for i, (pos, stroke, color) in enumerate(zip(downer_data[0], downer_data[1], downer_data[2])):
        ...
        img_downer_part.paste(
            downer_base[color], (0, 0), mask=mask_img_downer[i])
    ...
    # finish
    previmg = Image.new("RGBA", (max([upper_width, downer_width]) + leftmargin +
                        subset + 100, height + upmargin + 100), (255, 255, 255, 0))
    ...
    if upper_width > downer_width+subset:
        previmg.alpha_composite(
            tiltres[1], (upper_width + subset - downer_width, _round(height/2) + 50), (0, 0))
    else:
        previmg.alpha_composite(
    ...
    save_path = path.join(config.res, 'cacha', 'mkmeme', image_name)
