...
import config
from src.Services import GROUP_MEMBER, SUPERUSER, Service, GROUP_ADMIN, Service_Master, ALL_SERVICES, perm
...
[全局(开启|关闭)插件 插件名] -> 全局加载/删除一个插件
    ...
    使用示例:
        ...
        全局开启插件 setu
"""
sv = Service(['plugin_manager', '插件管理器'], sv_help,
             permission_change=SUPERUSER, permission_use=GROUP_ADMIN, visible=False)
svm = Service_Master()
...
async def plugin_list(session: CommandSession):
    ...
    enable_list = []
    disable_list = []
    if session.event['user_id'] in config.SUPERUSERS and session.event['message_type'] == 'private':
...
async def load_plugin(session):
    ...
    plugin_name = [x for x, y in svm.sv_list.items(
    ) if plugin_name in y.plugin_name]
    if not plugin_name:
